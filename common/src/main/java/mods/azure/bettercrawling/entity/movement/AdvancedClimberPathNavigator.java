...
public class AdvancedClimberPathNavigator<T extends Mob & IClimberEntity> extends AdvancedGroundPathNavigator<T> {
    ...
    public void tick() {
        ...
        if (!this.isDone()) {
            if (this.canUpdatePath()) {
                ...
                if (pos.y > targetPos.y && !this.mob.onGround() && Mth.floor(pos.x) == Mth.floor(
                        targetPos.x) && Mth.floor(pos.z) == Mth.floor(targetPos.z)) {
                    this.path.advance();
                }
            }
            ...
            if (!this.isDone()) {
                ...
                if (moveController instanceof ClimberMoveController && targetPoint instanceof DirectionalPathPoint && ((DirectionalPathPoint) targetPoint).getPathSide() != null) {
                    ((ClimberMoveController) moveController).setMoveTo(targetPos.x, targetPos.y, targetPos.z,
                            targetPoint.asBlockPos().relative(dir), ((DirectionalPathPoint) targetPoint).getPathSide(),
                            this.speedModifier);
                } else {
    ...
    public Vec3 getExactPathingTarget(BlockGetter blockaccess, BlockPos pos, Direction dir) {
        ...
        double offset = shape.isEmpty() ? sign /*undo offset if no collider*/ : (sign > 0 ? shape.min(
                axis) - 1 : shape.max(axis));
    ...
    protected void followThePath() {
        ...
        float maxDistanceToWaypointY = Math.max(1 /*required for e.g. slabs*/,
        ...
        for (int i = 4; i >= 0; i--) {
            if (this.path.getNextNodeIndex() + i < this.path.getNodeCount()) {
                ...
                if (isOnSameSideAsTarget && (isWaypointInReach || (i == 0 && this.mob.getNavigation().canCutCorner(
                        this.path.getNextNode().type) && this.isNextTargetInLine(pos, sizeX, sizeY, sizeZ, 1 + i)))) {
        ...
        if (this.path.getNextNodeIndex() + offset >= this.path.getNodeCount()) {
            ...
            if (!pos.closerThan(currentTarget, 2.0D)) {
                ...
                if (targetDir.dot(currentDir) > 0.0D) {
                    ...
                    return this.isSafeToStandAt(Mth.floor(nextTarget.x), Mth.floor(nextTarget.y),
                            Mth.floor(nextTarget.z), sizeX, sizeY, sizeZ, currentTarget, 0, 0, -1, ax, ay, az, invertY);
                }
    ...
    protected boolean canMoveDirectly(Vec3 start, Vec3 end/*, int sizeX, int sizeY, int sizeZ*/) {
        ...
        switch (this.verticalFacing.getAxis()) {
            case X:
                return this.isDirectPathBetweenPoints(start, end, sizeX, sizeY, sizeZ, Direction.Axis.Z,
                        Direction.Axis.X, Direction.Axis.Y, 0.0D, this.verticalFacing.getStepX() < 0);
            case Y:
                return this.isDirectPathBetweenPoints(start, end, sizeX, sizeY, sizeZ, Direction.Axis.X,
                        Direction.Axis.Y, Direction.Axis.Z, 0.0D, this.verticalFacing.getStepY() < 0);
            case Z:
                return this.isDirectPathBetweenPoints(start, end, sizeX, sizeY, sizeZ, Direction.Axis.Y,
                        Direction.Axis.Z, Direction.Axis.X, 0.0D, this.verticalFacing.getStepZ() < 0);
        }
        return false;
    }
    ...
    protected boolean isDirectPathBetweenPoints(Vec3 start, Vec3 end, int sizeX, int sizeY, int sizeZ, Direction.Axis ax, Direction.Axis ay, Direction.Axis az, double minDotProduct, boolean invertY) {
        ...
        if (dSq < 1.0E-8D) {
            ...
            if (!this.isSafeToStandAt(
                    unswizzle(bx, by, bz, ax, ay, az, Direction.Axis.X),
                    unswizzle(bx, by, bz, ax, ay, az, Direction.Axis.Y),
                    unswizzle(bx, by, bz, ax, ay, az, Direction.Axis.Z),
                    unswizzle(sizeX2, sizeY2, sizeZ2, ax, ay, az, Direction.Axis.X),
                    unswizzle(sizeX2, sizeY2, sizeZ2, ax, ay, az, Direction.Axis.Y),
                    unswizzle(sizeX2, sizeY2, sizeZ2, ax, ay, az, Direction.Axis.Z),
    ...
    protected boolean isSafeToStandAt(int x, int y, int z, int sizeX, int sizeY, int sizeZ, Vec3 start, double dx, double dz, double minDotProduct, Direction.Axis ax, Direction.Axis ay, Direction.Axis az, boolean invertY) {
        ...
        if (!this.isPositionClear(
                unswizzle(bx, y, bz, ax, ay, az, Direction.Axis.X), unswizzle(bx, y, bz, ax, ay, az, Direction.Axis.Y),
                unswizzle(bx, y, bz, ax, ay, az, Direction.Axis.Z),
    ...
    protected boolean isPositionClear(int x, int y, int z, int sizeX, int sizeY, int sizeZ, Vec3 start, double dx, double dz, double minDotProduct, Direction.Axis ax, Direction.Axis ay, Direction.Axis az) {
        for (BlockPos pos : BlockPos.betweenClosed(new BlockPos(x, y, z),
