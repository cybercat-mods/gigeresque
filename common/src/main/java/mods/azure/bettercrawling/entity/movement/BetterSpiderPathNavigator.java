...
public class BetterSpiderPathNavigator<T extends Mob & IClimberEntity> extends AdvancedClimberPathNavigator<T> {
	...
	@Override
	public boolean moveTo(@NotNull Entity entityIn, double speedIn) {
	...
	public void tick() {
		if(!this.isDone()) {
			...
			if(this.targetPosition != null && this.useVanillaBehaviour) {
				// FORGE: Fix MC-94054
				if(!this.targetPosition.closerThan(this.mob.blockPosition(), Math.max(this.mob.getBbWidth(), 1.0D)) && (!(this.mob.getY() > (double) this.targetPosition.getY()) || !(Constants.blockPos(this.targetPosition.getX(), this.mob.getY(), this.targetPosition.getZ())).closerThan(this.mob.blockPosition(), Math.max(
                        this.mob.getBbWidth(), 1.0D)))) {
					this.mob.getMoveControl().setWantedPosition(this.targetPosition.getX(),
                            this.targetPosition.getY(), this.targetPosition.getZ(), this.speedModifier);
				} else {
