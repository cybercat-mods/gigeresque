...
public class ClimberMoveController<T extends Mob & IClimberEntity> extends MoveControl {
    ...
    public void tick() {
        ...
        if (this.operation == Operation.STRAFE) {
            ...
            if (!this.isWalkableAtOffset(forwardVector.x * forward + strafeVector.x * strafe,
            ...
            if (this.side != null && this.block != null) {
                VoxelShape shape = this.mob.level().getBlockState(this.block).getCollisionShape(this.mob.level(),
                        this.block);
                ...
                if (aabb.intersects(blockAabb)) {
                    ...
                    double allowedOffset = shape.collide(offsetAxis,
                            aabb.move(-this.block.getX(), -this.block.getY(), -this.block.getZ()), offset);
            ...
            double verticalOffset = Math.abs(mainOffsetDir.getStepX() * dx) + Math.abs(
                    mainOffsetDir.getStepY() * dy) + Math.abs(mainOffsetDir.getStepZ() * dz);
            ...
            if (this.side != null && verticalOffset > reach - 0.05f && groundDir != this.side && groundDir.getAxis() != this.side.getAxis()) {
                ...
                if (hdsq < 0.707f) {
                    ...
                    if (hdsq < 0.1f) {
                        jumpDir = new Vec3(mainOffsetDir.getStepX(), mainOffsetDir.getStepY(),
                                mainOffsetDir.getStepZ());
                    }
                }
            }
            ...
            if (targetDist < 0.0001D) {
                ...
                if (jumpDir != null) {
                    ...
                    if (jumpController instanceof ClimberJumpController climberJumpController) {
                        climberJumpController.setJumping(jumpDir);
                    }
                } else {
                    this.mob.setSpeed((float) speed);
                }
