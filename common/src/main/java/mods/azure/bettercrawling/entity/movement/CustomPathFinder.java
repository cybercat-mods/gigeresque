...
public class CustomPathFinder extends PathFinder {
    ...
    public static final Heuristic DEFAULT_HEURISTIC = (start, end, isTargetHeuristic) -> start.distanceManhattan(
            end); //distanceManhattan
    ...
    private Path findPath(Node start, Map<Target, BlockPos> checkpointsMap, float maxDistance, int checkpointRange, float maxExpansionsMultiplier) {
        ...
        while (!this.path.isEmpty() && ++expansions < maxExpansions) {
            ...
            if (openPathPoint.distanceTo(start) < maxDistance) {
                ...
                for (int i = 0; i < numOptions; ++i) {
                    ...
                    float costHeuristic = openPathPoint.distanceTo(
                            successorPathPoint); //TODO Replace with cost heuristic
                    ...
                    if (successorPathPoint.walkedDistance < maxDistance && (!successorPathPoint.inOpenSet() || totalSuccessorPathCost < successorPathPoint.g)) {
                        ...
                        successorPathPoint.h = this.computeHeuristic(successorPathPoint,
                                checkpoints); //TODO Vanilla's 1.5 multiplier is too greedy :( Move to custom heuristic stuff
        ...
        if (!reachedCheckpoints.isEmpty()) {
            ...
            path = reachedCheckpoints.stream().map(
                    checkpoint -> this.createPath(checkpoint.getBestNode(), checkpointsMap.get(checkpoint), true)).min(
                    Comparator.comparingInt(Path::getNodeCount));
        } else {
            //Use lowest cost path towards any checkpoint
            path = checkpoints.stream().map(
                    checkpoint -> this.createPath(checkpoint.getBestNode(), checkpointsMap.get(checkpoint), false)).min(
                    Comparator.comparingDouble(
                            Path::getDistToTarget /*TODO Replace calculation with cost heuristic*/).thenComparingInt(
                            Path::getNodeCount));
        }
