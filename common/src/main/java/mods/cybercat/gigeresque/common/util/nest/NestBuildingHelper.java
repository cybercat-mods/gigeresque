...
public record NestBuildingHelper() {
            ...
            for (var z = -1; z <= 1; z++) {
                for (var y = -1; y <= 3; y++) {
                    ...
                    if (level.getLightEmission(pos) < 6) {
                        var resinBlock = GigBlocks.NEST_RESIN.get().defaultBlockState().setValue(NestResinBlock.LAYERS,
                                Math.min(8, level.getRandom().nextInt(7) + 1));
                        ...
                        if (nestBlockData.isCorner() && !level.getBlockState(blockPos).is(GigTags.DUNGEON_BLOCKS))
                            level.setBlockAndUpdate(blockPos, GigBlocks.NEST_RESIN_WEB_CROSS.get().defaultBlockState());

                        if (nestBlockData.isWall() || nestBlockData.isCeiling()) {
                            var nestResinWebState = GigBlocks.NEST_RESIN_WEB.get().defaultBlockState().setValue(
                                    NestResinWebBlock.UP, nestBlockData.hasUpCoverage()).setValue(
                                    NestResinWebBlock.NORTH, nestBlockData.hasNorthCoverage()).setValue(
                                    NestResinWebBlock.SOUTH, nestBlockData.hasSouthCoverage()).setValue(
                                    NestResinWebBlock.EAST, nestBlockData.hasEastCoverage()).setValue(
                                    NestResinWebBlock.WEST, nestBlockData.hasWestCoverage()).setValue(
                                    NestResinWebBlock.VARIANTS, NestResinWebVariant.values()[level.getRandom().nextInt(
    ...
    private static NestBlockData getNestBlockData(Level world, BlockPos blockPos) {
        ...
        blockStates.forEach(it -> {
            ...
            if (isOpaqueCube) if (!isResinBlock(it.getSecond().getBlock())) {
                if (it.getFirst() != upPos && it.getFirst() != downPos) horizontalCoverage.getAndIncrement();
                fullCoverage.getAndIncrement();
            } else cornerCoverage.getAndIncrement();
        });

        var isFloor = actualState.isAir() && upState.isAir() && !isResinBlock(
                downState.getBlock()) && downState.isSolidRender(world, downPos) && world.getLightEmission(
                blockPos) < 10;
        var isWall = !isFloor && actualState.isAir() && (upState.isAir() || isResinBlock(
                upState.getBlock())) && (1 <= horizontalCoverage.get() && horizontalCoverage.get() <= 2) && world.getLightEmission(
                blockPos) < 10;
        ...
        var isCeiling = actualState.isAir() && (downState.isAir() || isResinBlock(
                downState.getBlock())) && upState.isSolidRender(world, downPos) && !isCorner && world.getLightEmission(
                blockPos) < 10;

        return new NestBlockData(fullCoverage.get(), isCorner, isFloor, isCeiling, isWall, /* upCoverage */
                ...
                eastState.isSolidRender(world, eastPos), /* westCoverage */ westState.isSolidRender(world, westPos));
    }
}
