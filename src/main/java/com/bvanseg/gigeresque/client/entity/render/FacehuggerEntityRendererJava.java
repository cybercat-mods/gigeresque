...
import com.bvanseg.gigeresque.client.entity.model.FacehuggerEntityModelJava;
...
public class FacehuggerEntityRendererJava extends GeoEntityRenderer<FacehuggerEntityJava> {
    private final HashMap<EntityType<?>, TransformDataGenerator> headDistances = new HashMap<>();

    public FacehuggerEntityRendererJava(EntityRendererFactory.Context context) {
        super(context, new FacehuggerEntityModelJava());
        this.shadowRadius = 0.2f;
        headDistances.put(EntityType.SHEEP, (facehugger, host) -> new TransformData(
                0.0,
                ((double) host.getEyeHeight(host.getPose())) - host.getMountedHeightOffset() - facehugger.getWidth() + 0.4,
                host.getWidth() - ((double) facehugger.getHeight()) - 0.1,
                0.385,
                calcStandardOffsetY(facehugger) - 0.05
        ));
        headDistances.put(EntityType.COW, (facehugger, host) -> new TransformData(
                0.0,
                ((double) host.getEyeHeight(host.getPose())) - host.getMountedHeightOffset() - facehugger.getWidth() + 0.4,
                host.getWidth() - ((double) facehugger.getHeight()) - 0.1,
                0.41,
        ...
        headDistances.put(EntityType.PIG, (facehugger, host) -> new TransformData(
                0.0,
                ((double) host.getEyeHeight(host.getPose())) - host.getMountedHeightOffset() - facehugger.getWidth() + 0.4,
                host.getWidth() - ((double)facehugger.getHeight()) - 0.1,
        ...
        headDistances.put(EntityType.WOLF, (facehugger, host) -> new TransformData(
                0.0,
                ((double)host.getEyeHeight(host.getPose())) - host.getMountedHeightOffset() - facehugger.getWidth() + 0.4,
                host.getWidth() - ((double)facehugger.getHeight()) - 0.1,
                0.54,
        ...
        headDistances.put(EntityType.VILLAGER, (facehugger, host) -> new TransformData(
                0.0,
                0.0,
                0.0,
                0.36,
    ...
    @Override
    protected void applyRotations(FacehuggerEntityJava facehugger, MatrixStack matrixStackIn, float ageInTicks, float rotationYaw, float partialTicks) {
    ...
    private TransformData getTransformData(FacehuggerEntityJava facehugger, Entity host) {
        return headDistances.computeIfAbsent(host.getType(), entityType -> (facehugger1, host1) -> new TransformData(
                0.0,
                0.0,
                0.0,
                ((double) host.getEyeHeight(host.getPose())) - host.getMountedHeightOffset() - facehugger.getWidth() + host.getWidth(),
                calcStandardOffsetY(facehugger)
        )).invoke(facehugger, host);
    }

    private double calcStandardOffsetY(FacehuggerEntityJava facehugger) {
    ...
    private interface TransformDataGenerator {
        public TransformData invoke(FacehuggerEntityJava facehugger, Entity host);
    }
}
