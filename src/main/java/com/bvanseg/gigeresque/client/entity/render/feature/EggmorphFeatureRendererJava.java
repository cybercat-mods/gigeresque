...
import com.bvanseg.gigeresque.client.entity.texture.EggmorphLayerTextureJava;
...
public class EggmorphFeatureRendererJava<T extends Entity, M extends EntityModel<T>> extends FeatureRenderer<T, M> {
    public EggmorphFeatureRendererJava(FeatureRendererContext<T, M> context) {
        super(context);
    }

    private static final HashMap<Identifier, EggmorphLayerTextureJava> textureCache = new HashMap<Identifier, EggmorphLayerTextureJava>();
    ...
    public static <T extends Entity> void renderEggmorphedModel(
        ...
        var progress = 1 - ((Eggmorphable) entity).getTicksUntilEggmorphed() / ((float) ConstantsJava.EGGMORPH_DURATION);
    ...
    public static EggmorphLayerTextureJava getEggmorphLayerTexture(Identifier texture) {
        return textureCache.computeIfAbsent(texture, identifier ->
                new EggmorphLayerTextureJava(
