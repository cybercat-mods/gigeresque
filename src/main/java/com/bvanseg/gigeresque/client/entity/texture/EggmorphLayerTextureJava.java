...
public class EggmorphLayerTextureJava implements AutoCloseable {
    private static final Identifier layerTexture = new Identifier(GigeresqueJava.MOD_ID, "textures/misc/eggmorph_overlay.png");
    public RenderLayer renderLayer;
    private NativeImageBackedTexture texture;
    public EggmorphLayerTextureJava(TextureManager textureManager, ResourceManager resourceManager, Identifier base) {
        ...
        MinecraftClient.getInstance().execute(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    var layerImage = NativeImage.read(
                            resourceManager.getResource(layerTexture).getInputStream()
                    );
                    texture = new NativeImageBackedTexture(new NativeImage(layerImage.getWidth(), layerImage.getHeight(), true));
                    ...
                    GigeresqueJava.LOGGER.error("Could not generate eggmorph texture for $base.", e);
                }
            }
        });
    }
