...
public class NestResinBlockJava extends Block {
    public NestResinBlockJava(Settings settings) {
        super(settings);
    ...
    public VoxelShape getCollisionShape(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
        return context instanceof EntityShapeContext && ((EntityShapeContext) context).getEntity() instanceof AlienEntityJava ?
    ...
    public boolean canPlaceAt(BlockState state, WorldView world, BlockPos pos) {
        ...
        if (!isIce && !isPackedIce && !isBarrier) {
            if (!isHoney && !isSoulSand) {
                return isFaceFullSquare(blockState.getCollisionShape(world, pos.down()), Direction.UP) || blockState.isOf(this) && blockState.get(LAYERS) == 8;
            } else {
                return true;
            }
        } else {
