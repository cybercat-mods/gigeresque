...
public class NestResinWebBlock extends Block {
    ...
    public static final EnumProperty<NestResinWebVariant> VARIANTS = EnumProperty.of("nest_resin_web_variant", NestResinWebVariant.class);
    ...
    public NestResinWebBlock(Settings settings) {
        ...
        shapesByState = getStateManager().getStates().stream().collect(
                Collectors.toMap(Function.identity(), NestResinWebBlock::getShapeForState)
        );
    }
        ...
        for (Direction direction : placementDirections) {
            if (direction != Direction.DOWN) {
                ...
                if (!bl2 && shouldHaveSide(ctx.getWorld(), ctx.getBlockPos(), direction)) {
                    return blockState2.with(booleanProperty, true).with(VARIANTS, Arrays.stream(NestResinWebVariant.values()).toList().get(new Random().nextInt(NestResinWebVariant.values().length)));
                }
            }
        }
