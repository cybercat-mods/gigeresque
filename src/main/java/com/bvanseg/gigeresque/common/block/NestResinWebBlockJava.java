...
public class NestResinWebBlockJava extends Block {
    ...
    public static final EnumProperty<NestResinWebVariantJava> VARIANTS = EnumProperty.of("nest_resin_web_variant", NestResinWebVariantJava.class);
    ...
    public NestResinWebBlockJava(Settings settings) {
        super(settings);
        setDefaultState(getStateManager().getDefaultState()
                ...
                .with(SOUTH, false)
                .with(WEST, false)
                .with(VARIANTS, NestResinWebVariantJava.ONE)
        );
        shapesByState = getStateManager().getStates().stream().collect(
                Collectors.toMap(Function.identity(), NestResinWebBlockJava::getShapeForState)
        );
    }
        ...
        for (Direction direction : placementDirections) {
            if (direction != Direction.DOWN) {
                ...
                if (!bl2 && shouldHaveSide(ctx.getWorld(), ctx.getBlockPos(), direction)) {
                    return blockState2.with(booleanProperty, true).with(VARIANTS, Arrays.stream(NestResinWebVariantJava.values()).toList().get(new Random().nextInt(NestResinWebVariantJava.values().length)));
                }
            }
        }
