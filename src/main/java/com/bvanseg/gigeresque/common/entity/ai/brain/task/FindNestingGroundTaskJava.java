...
import com.bvanseg.gigeresque.common.block.NestResinBlockJava;
...
public class FindNestingGroundTaskJava extends Task<AdultAlienEntityJava> {
    private final double speed;
    private boolean hasReachedNestingGround = false;
    private BlockPos targetPos = null;

    public FindNestingGroundTaskJava(double speed) {
    ...
    protected boolean shouldRun(ServerWorld serverWorld, AdultAlienEntityJava alien) {
        return alien.getGrowth() == alien.getMaxGrowth() && (!alien.getBrain().hasMemoryModule(MemoryModuleType.HOME) || !hasReachedNestingGround);
    }
            ...
            if (!canMoveTo) {
                ...
                for (int i = 0; i < 20; i++) {
                    ...
                    if (world.getBlockState(newPos).isAir() && world.getBlockState(downPos).isOpaque()) {
                        ...
                        if (alien.getNavigation().startMovingTo(targetPos.getX(), targetPos.getY(), targetPos.getZ(), speed)) {
                            break;
                        }
                    }
                }
            ...
            if (targetPos.isWithinDistance(alien.getPos(), 4.0)) {
                ...
                for (int x = -1; x <= 1; x++) {
                    for (int z = -1; z <= 1; z++) {
                        ...
                        boolean travelUp = !(world.getBlockState(resinPos).isAir() && world.getBlockState(downPos).isAir());
                        ...
                        if (topState.isAir()) {
                            ...
                            if (downState.getBlock() == BlocksJava.NEST_RESIN) {
                                world.setBlockState(
                                        downPos,
                                        downState.with(NestResinBlockJava.LAYERS, downState.get(NestResinBlockJava.LAYERS) + 1)
                                );
                            } else if (downState.isOpaqueFullCube(world, downPos)) {
                                world.setBlockState(resinPos, BlocksJava.NEST_RESIN.getDefaultState());
                            }
                        } else if (world.getBlockState(resinPos) == BlocksJava.NEST_RESIN.getDefaultState() &&
                                topState.isOpaqueFullCube(world, resinPos)
                        ) {
                            world.setBlockState(
                                    resinPos,
                                    topState.with(NestResinBlockJava.LAYERS, topState.get(NestResinBlockJava.LAYERS) + 1)
                            );
                        }
                    }
                }
    ...
    @Override
    public void finishRunning(ServerWorld world, AdultAlienEntityJava entity, long time) {
        super.finishRunning(world, entity, time);
        hasReachedNestingGround = false;
    }

    private Vec3d locateShadedPos(AlienEntityJava entity) {
