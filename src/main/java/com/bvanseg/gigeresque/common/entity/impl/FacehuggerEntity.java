...
public class FacehuggerEntity extends AlienEntity implements IAnimatable {
    ...
    private static final List<SensorType<? extends Sensor<? super FacehuggerEntity>>> SENSOR_TYPES = List.of(
    ...
    public void tick() {
        ...
        if (isAttachedToHost()) {
            ...
            if (host != null) {
                if (ticksAttachedToHost > Constants.TPS * 3 && host.doesNotHaveParasite()) {
                    ...
                    SoundUtil.playServerSound(world, null, this.getBlockPos(), Sounds.FACEHUGGER_IMPLANT, SoundCategory.NEUTRAL, 0.5f);
                    setIsInfertile(true);
                }
            }
        } else {
        ...
        var vehicle = this.getVehicle();
        if (vehicle != null && ((vehicle instanceof PlayerEntity && ((PlayerEntity) vehicle).isCreative()) || ticksAttachedToHost >= Constants.TPM * 5)) {
