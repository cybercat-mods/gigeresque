...
public class Items implements GigeresqueInitializer {
    ...
    public static final BucketItem BLACK_FLUID_BUCKET = new BucketItem(Fluids.BLACK_FLUID_STILL, new Item.Settings().recipeRemainder(net.minecraft.item.Items.BUCKET).maxCount(1).group(ItemGroups.GENERAL));

    public static final SurgeryKitItem SURGERY_KIT = new SurgeryKitItem(new Item.Settings().maxDamage(4).group(ItemGroups.GENERAL));
    ...
    private void registerSpawnEgg(
        ...
        Registry.register(
                Registry.ITEM,
                new Identifier(Gigeresque.MOD_ID, "%s_spawn_egg".formatted(path)),
                new SpawnEggItem(entityType, primaryColor, secondaryColor, new Item.Settings().group(ItemGroups.GENERAL))
        );
    }
}
