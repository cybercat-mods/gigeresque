...
public class GigPOIs {
    ...
    public static final Supplier<PoiType> GUNSMITH_POI = registerPoiType(
        "nests",
        () -> new PoiType(PointOfInterestTypesInvoker.invokeGetBlockStates(GigBlocks.NEST_RESIN_WEB_CROSS), 1, 30)
    );
    ...
    public static Supplier<PoiType> registerPoiType(String name, Supplier<PoiType> poiType) {
        ...
        PointOfInterestTypesInvoker.invokeRegisterBlockStates(
            ...
            PoiTypes.getBlockStates(GigBlocks.NEST_RESIN_WEB_CROSS)
        );
        return () -> registry;
    }
}
