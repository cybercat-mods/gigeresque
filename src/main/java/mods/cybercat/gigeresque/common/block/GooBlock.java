...
public class GooBlock extends FallingBlock implements Waterloggable {
	...
	public void randomDisplayTick(BlockState state, World world, BlockPos pos, Random random) {
		for (int i = 0; i < (getThickness(state) * 2) + 1; i++) {
			...
			world.addImportantParticle(Particles.GOO, d, e, f, 0.0, 0.0, 0.0);
		}
	...
	private void dealAcidDamage(BlockState state, Entity entity) {
		if (entity instanceof LivingEntity && !(entity instanceof AlienEntity) && !(entity instanceof WitherEntity))
			((LivingEntity) entity).addStatusEffect(new StatusEffectInstance(GigStatusEffects.DNA,
					Gigeresque.config.miscellaneous.getgooEffectTickTimer(), 0, true, true));
	}
