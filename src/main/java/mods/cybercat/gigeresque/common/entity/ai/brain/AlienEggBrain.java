...
public class AlienEggBrain extends ComplexBrain<AlienEggEntity> {
	...
	private void tryHatch() {
		List<AlienEggEntity> nearestEggs = brain.getOptionalMemory(MemoryModuleTypes.NEAREST_EGGS)
				.orElse(Collections.emptyList());
		List<FacehuggerEntity> nearestFacehuggers = brain.getOptionalMemory(MemoryModuleTypes.NEAREST_FACEHUGGERS)
				.orElse(Collections.emptyList()).stream().filter(it -> it.ticksAttachedToHost < 0).toList();
		List<LivingEntity> nearestHosts = brain.getOptionalMemory(MemoryModuleTypes.NEAREST_HOSTS)
				.orElse(Collections.emptyList());
		List<AlienEggEntity> nearestHatchingEggs = nearestEggs.stream()
				.filter(it -> it.isHatching() || (it.isHatched() && it.hasFacehugger())).toList();
