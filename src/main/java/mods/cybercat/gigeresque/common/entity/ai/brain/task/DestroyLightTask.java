...
public class DestroyLightTask extends Task<AlienEntity> {
	...
	protected void run(ServerWorld serverWorld, AlienEntity alien, long l) {
		var lightSourceLocation = alien.getBrain().getOptionalMemory(MemoryModuleTypes.NEAREST_LIGHT_SOURCE)
				.orElse(null);
			...
			if (!world.isClient) {
				for (int i = 0; i < 20; i++) {
					...
					((ServerWorld) world).spawnParticles(ParticleTypes.POOF,
							((double) lightSourceLocation.getX()) + 0.5, lightSourceLocation.getY(),
							((double) lightSourceLocation.getZ()) + 0.5, 1, e, f, g, 0.15000000596046448);
				}
	...
	private void startMovingToTarget(AlienEntity alien, BlockPos targetPos) {
		alien.getNavigation().startMovingTo(((double) ((float) targetPos.getX())) + 0.5, targetPos.getY(),
				((double) ((float) targetPos.getZ())) + 0.5, speed);
	}
}
