...
public class BuildNestTask<E extends AdultAlienEntity> extends DelayedBehaviour<E> {
	...
	protected boolean checkExtraStartConditions(ServerLevel level, E alien) {
		return !alien.isAggressive() && !alien.isVehicle() && alien.getGrowth() == alien.getMaxGrowth() && !alien.getLevel().canSeeSky(alien.blockPosition()) && !alien.getLevel().getBlockState(alien.blockPosition()).is(Blocks.SOUL_SAND) && alien.getLevel().getBrightness(LightLayer.SKY, alien.blockPosition()) <= 5;
	}

	@Override
	protected void doDelayedAction(E alien) {
		var list2 = alien.getLevel().getBlockStatesIfLoaded(alien.getBoundingBox().inflate(4.0, 4.0, 4.0));
		if (list2.noneMatch(NEST) && !alien.getLevel().canSeeSky(alien.blockPosition()) && alien.getLevel().getBrightness(LightLayer.SKY, alien.blockPosition()) <= 5)
