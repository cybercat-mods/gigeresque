...
public class FacehuggerPounceTask<E extends FacehuggerEntity> extends CustomDelayedMeleeBehaviour<E> {
	...
	protected boolean checkExtraStartConditions(ServerLevel level, E entity) {
		...
		return GigEntityUtils.faceHuggerTest(this.target, entity) && entity.isWithinMeleeAttackRange(this.target) && !this.target.getLevel().getBlockStates(this.target.getBoundingBox().inflate(1)).anyMatch(state -> state.is(GIgBlocks.NEST_RESIN_WEB_CROSS));
	}
