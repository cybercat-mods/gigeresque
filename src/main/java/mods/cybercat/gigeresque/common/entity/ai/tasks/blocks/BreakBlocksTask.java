...
public class BreakBlocksTask<E extends AlienEntity> extends DelayedBehaviour<E> {
    ...
    protected boolean checkExtraStartConditions(ServerLevel level, E entity) {
        var notinWater = !(entity.level()
            .getFluidState(entity.blockPosition())
            .is(
                Fluids.WATER
            ) && entity.level().getFluidState(entity.blockPosition()).getAmount() >= 8);
        var canGrief = entity.level().getGameRules().getBoolean(GameRules.RULE_MOBGRIEFING);
        return !entity.isCrawling() && !entity.isTunnelCrawling() && !entity.isDeadOrDying() && !entity.isPassedOut() && notinWater
            && canGrief;
    }
    ...
    protected void doDelayedAction(E entity) {
        for (
            var testPos : BlockPos.betweenClosed(
                entity.blockPosition().relative(entity.getDirection()),
                    ...
                    if (entity.level().isClientSide()) {
                        for (var i = 2; i < 10; i++) {
                            entity.level()
                                .addAlwaysVisibleParticle(
                                    Particles.ACID,
                                    entity.getX() + ((entity.getRandom().nextDouble() / 2.0) - 0.5) * (entity.getRandom().nextBoolean()
                                        ? -1
                                        : 1),
                                    entity.getEyeY() - ((entity.getEyeY() - entity.blockPosition().getY()) / 2.0),
                                    entity.getZ() + ((entity.getRandom().nextDouble() / 2.0) - 0.5) * (entity.getRandom().nextBoolean()
                                        ? -1
                                        : 1),
                                    0.0,
                                    -0.15,
                        ...
                        entity.level()
                            .playLocalSound(
                                ...
                                0.9f + entity.getRandom().nextFloat() * 0.15f,
                                false
                            );
                    }
                } else if (
                    !entity.isCrawling() && !entity.isTunnelCrawling() && !entity.isVehicle() && !state.is(
                        GigTags.ACID_RESISTANT
                    ) && !state.isAir() && (entity.getHealth() >= (entity.getMaxHealth() * 0.50)) && useAcidBlood
                ) {
                    if (!entity.level().isClientSide) {
