		...
		return BrainActivityGroup
				.idleTasks(
						new BuildNestTask(90).stopIf(target -> (this.isAggressive() || this.isVehicle()
								|| this.isStatis() || this.entityData.get(FLEEING_FIRE).booleanValue() == true)),
						...
						new OneRandomBehaviour<>(new SetRandomWalkTarget<>().speedModifier(1.05f)
								.startCondition(entity -> !this.isExecuting()).stopIf(entity -> this.isExecuting()),
								new Idle<>()
			...
			return event.setAndContinue(this.isStatis() == true || this.isNoAi() ? GigAnimationsDefault.STATIS_ENTER
					: this.isSearching && this.isVehicle() ? GigAnimationsDefault.AMBIENT
							: this.isExecuting() == true && this.isVehicle() ? GigAnimationsDefault.EXECUTION_CARRY
									: this.wasEyeInWater ? GigAnimationsDefault.IDLE_WATER
