...
/* Credit to Nyfaria
 * https://github.com/Nyfaria/Spiders-2.0-Fabric/
*/
public class CollisionSmoothingUtil {
	...
	private static float sampleSdf(float[] erx, float[] ery, float[] erz, float[] ecx, float[] ecy, float[] ecz,
			int count, float px, float py, float pz, float pnx, float pny, float pnz, float x, float y, float z,
			float smoothingRange, float invSmoothingRange) {
		float sdfDst = 0.0f;
	...
	private static class BoxConsumer implements VoxelShapes.BoxConsumer {
		...
		public void consume(double minX, double minY, double minZ, double maxX, double maxY, double maxZ) {
			if (this.size == this.capacity) {
				this.capacity = (int) Math.max(Math.min((long) this.capacity + (this.capacity >> 1),
						it.unimi.dsi.fastutil.Arrays.MAX_ARRAY_SIZE), this.capacity);
	...
	@Nullable
	public static Pair<Vec3d, Vec3d> findClosestPoint(Consumer<VoxelShapes.BoxConsumer> consumer, Vec3d pp, Vec3d pn,
			float smoothingRange, float boxScale, float dx, int iters, float threshold, Vec3d p) {
		...
		return findClosestPoint(boxConsumer.erx, boxConsumer.ery, boxConsumer.erz, boxConsumer.ecx, boxConsumer.ecy,
				boxConsumer.ecz, boxConsumer.size, pp, pn, smoothingRange, boxScale, dx, iters, threshold, p);
	}

	@Nullable
	public static Pair<Vec3d, Vec3d> findClosestPoint(List<Box> boxes, Vec3d pp, Vec3d pn, float smoothingRange,
			float boxScale, float dx, int iters, float threshold, Vec3d p) {
		if (boxes.isEmpty()) {
			return null;
		}
		...
		return findClosestPoint(erx, ery, erz, ecx, ecy, ecz, boxes.size(), pp, pn, smoothingRange, boxScale, dx, iters,
				threshold, p);
	}

	@Nullable
	private static Pair<Vec3d, Vec3d> findClosestPoint(float[] erx, float[] ery, float[] erz, float[] ecx, float[] ecy,
			float[] ecz, int count, Vec3d pp, Vec3d pn, float smoothingRange, float boxScale, float dx, int iters,
			float threshold, Vec3d p) {
		...
		for (int j = 0; j < iters; j++) {
			float dst = sampleSdf(erx, ery, erz, ecx, ecy, ecz, count, plx, ply, plz, pnx, pny, pnz, px, py, pz,
					smoothingRange, invSmoothingRange);

			float fx1 = sampleSdf(erx, ery, erz, ecx, ecy, ecz, count, plx, ply, plz, pnx, pny, pnz, px + dx, py, pz,
					smoothingRange, invSmoothingRange);
			float fy1 = sampleSdf(erx, ery, erz, ecx, ecy, ecz, count, plx, ply, plz, pnx, pny, pnz, px, py + dx, pz,
					smoothingRange, invSmoothingRange);
			float fz1 = sampleSdf(erx, ery, erz, ecx, ecy, ecz, count, plx, ply, plz, pnx, pny, pnz, px, py, pz + dx,
					smoothingRange, invSmoothingRange);
			...
			if (Float.isNaN(gx) || Float.isNaN(gy) || Float.isNaN(gz) || Double.isNaN(px) || Double.isNaN(py)
					|| Double.isNaN(pz)) {
				return null;
			}
