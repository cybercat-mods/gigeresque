...
public class GigVibrationListener implements GameEventListener {
	...
	public static Codec<GigVibrationListener> codec(GigVibrationListenerConfig vibrationListenerConfig) {
		return RecordCodecBuilder.create(instance -> instance
				.group((PositionSource.CODEC.fieldOf("source")).forGetter(vibrationListener -> ((GigVibrationListener) vibrationListener).listenerSource), (ExtraCodecs.NON_NEGATIVE_INT.fieldOf("range")).forGetter(vibrationListener -> ((GigVibrationListener) vibrationListener).listenerRange), GigVibrationInfo.CODEC.optionalFieldOf("event").forGetter(vibrationListener -> Optional.ofNullable(((GigVibrationListener) vibrationListener).currentVibration)),
						(GigVibrationSelector.CODEC.fieldOf("selector")).forGetter(vibrationListener -> ((GigVibrationListener) vibrationListener).selectionStrategy), (ExtraCodecs.NON_NEGATIVE_INT.fieldOf("event_delay")).orElse(0).forGetter(vibrationListener -> ((GigVibrationListener) vibrationListener).travelTimeInTicks))
				.apply(instance, (positionSource, integer, optional, vibrationSelector, integer2) -> new GigVibrationListener((PositionSource) positionSource, (int) integer, vibrationListenerConfig, optional.orElse(null), (GigVibrationSelector) vibrationSelector, (int) integer2)));
	}

	public GigVibrationListener(PositionSource positionSource, int i, GigVibrationListenerConfig vibrationListenerConfig, @Nullable GigVibrationInfo vibrationInfo, GigVibrationSelector vibrationSelector, int j) {
	...
	public GigVibrationListener(PositionSource positionSource, int i, GigVibrationListenerConfig vibrationListenerConfig) {
	...
	public void scheduleVibration(ServerLevel serverLevel, GameEvent gameEvent, GameEvent.Context context, Vec3 vec3, Vec3 vec32) {
		this.selectionStrategy.addCandidate(new GigVibrationInfo(gameEvent, (float) vec3.distanceTo(vec32), vec3, context.sourceEntity()), serverLevel.getGameTime());
	}
