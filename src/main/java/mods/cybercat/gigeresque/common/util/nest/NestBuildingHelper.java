            ...
            for (var z = -1; z <= 1; z++) {
                for (var y = -1; y <= 3; y++) {
                    ...
                    if (level.getLightEmission(pos) < 6) {
                        var resinBlock = GigBlocks.NEST_RESIN.defaultBlockState().setValue(NestResinBlock.LAYERS,
                        ...
                        if (nestBlockData.isWall() || nestBlockData.isCeiling()) {
                            var nestResinWebState = GigBlocks.NEST_RESIN_WEB.defaultBlockState().setValue(
