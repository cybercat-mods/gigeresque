...
class NearestAlienTargetSensor : Sensor<LivingEntity>() {
    ...
    override fun getOutputMemoryModules(): Set<MemoryModuleType<*>> {
        return ImmutableSet.of(
            MemoryModuleTypes.EGGMORPH_TARGET, MemoryModuleType.ATTACK_TARGET, MemoryModuleType.VISIBLE_MOBS, MemoryModuleType.NEAREST_VISIBLE_PLAYER
        )
    }
    ...
    override fun sense(world: ServerWorld, entity: LivingEntity) {
        ...
        nearestPlayer.getOrNull()?.let { player ->
            if (!(player as Host).hasParasite()) {
                brain.remember(MemoryModuleType.ATTACK_TARGET, player)
                return
            }
        }
        ...
        nearestVisibleMobs.firstOrNull {
            if (it.isEggmorphable() && !it.hasVehicle()) {
                brain.remember(MemoryModuleTypes.EGGMORPH_TARGET, it)
                return@firstOrNull false
            }
            if (ConfigAccessor.isTargetBlacklisted(entity, it)) return@firstOrNull false
            if (ConfigAccessor.isTargetWhitelisted(entity, it)) return@firstOrNull true
